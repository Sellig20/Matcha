...
export class chatController {
  static async postMessages(req: Request, res: Response) {
    try {
      ...
      if (!userId) userId = req.body.sender_id;
      let interlocuteur_id = req.query.receiver_id;
      if (!interlocuteur_id) interlocuteur_id = req.body.receiver_id;
      ...
      res.status(204).json({
        message: `chatController.ts | Error during posting message text : ${error}`,
      });
    }
  }
  ...
  static async readMessages(req: Request, res: Response) {
    try {
      ...
      const result_me_sender = await userSignupModel.readMyMessages(
        "sender_id",
        Number(userId)
      );
      ...
      const result_user_sender = await userSignupModel.readMyMessages(
        "receiver_id",
        Number(userId)
      );
      ...
      if (result_user_sender) {
        for (let i = 0; i < result_user_sender.length; i++) {
          if (
            result_user_sender[i].receiver_id === Number(userId) &&
      ...
      if (result_me_sender && result_user_sender) {
        return res.status(200).json({
          message: `We both ${userId} | ${interlocuteur_id} sent message`,
          myMsg: tabPushMyMessages,
          itsMsg: tabPushItsMessages,
        });
      } else if (result_me_sender) {
        return res.status(200).json({
          message: `Only I ${userId} sent message`,
          myMsg: tabPushMyMessages,
        });
      } else if (result_user_sender) {
        return res.status(200).json({
          message: `Only this person ${interlocuteur_id} sent message`,
          itsMsg: tabPushItsMessages,
        });
      } else {
        res
          .status(200)
          .json({ message: `ChatController.ts | readMessages | Error` });
      }
    } catch (error) {
      res.status(204).json({
        message: `chatController.ts | Error during posting message text : ${error}`,
