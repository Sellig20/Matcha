...
export class userPicturesModel {
    static async createPictures(newPictures: UsersPicturesCreate) {
        const res = await orm?.create<"users_pictures">("users_pictures", newPictures)
        return res;
    }
    ...
    static async updatePictures(id: number, updateData: Partial<UsersPicturesCreate>) {
        const res = await orm?.update<"users_pictures">("users_pictures", id, updateData)
        return res;
    }
